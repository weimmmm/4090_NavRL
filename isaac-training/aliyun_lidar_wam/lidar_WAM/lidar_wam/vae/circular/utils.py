...
class Conv2d(torch.nn.Conv2d):
    ...
    def _conv_forward(self, input: torch.Tensor, weight: torch.Tensor, bias: Optional[torch.Tensor]):
        ...
        # Match the input dtype to the convolution parameters for mixed-precision execution.
        input = input.to(self.bias.dtype) 
...
class Downsample2D(torch.nn.Module):
    ...
    def __init__(
        ...
        # Preserve both historical convolution names for checkpoint compatibility.
        if name == "conv":
            self.Conv2d_0 = conv
            self.conv = conv
        elif name == "Conv2d_0":
