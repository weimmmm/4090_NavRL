...
except ImportError:  # Keep the standalone PPU runner usable without einops.
    def rearrange(x, pattern, **kwargs):
        if pattern == "b s (n d) -> b n s d":
            n = int(kwargs["n"])
            b, s, width = x.shape
            return x.reshape(b, s, n, width // n).permute(0, 2, 1, 3)
        ...
        if pattern == "b (f h w) (x y z c) -> b c (f x) (h y) (w z)":
            f, h, w = (int(kwargs[key]) for key in ("f", "h", "w"))
            px, py, pz = (int(kwargs[key]) for key in ("x", "y", "z"))
            b, tokens, packed = x.shape
            c = packed // (px * py * pz)
            if tokens != f * h * w or packed != px * py * pz * c:
                raise ValueError("Invalid Wan unpatchify dimensions")
            return (x.reshape(b, f, h, w, px, py, pz, c)
                     .permute(0, 7, 1, 4, 2, 5, 3, 6)
                     .reshape(b, c, f * px, h * py, w * pz))
