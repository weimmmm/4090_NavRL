...
class PPO(TensorDictModuleBase):
    def __init__(
        ...
        self.n_agents = 1
        try:
            command_spec = action_spec[("agents", "action")]
        except (KeyError, TypeError, IndexError):
            command_spec = action_spec
        self.action_dim = command_spec.shape[-1]
        if self.action_dim != 3:
            raise ValueError(
                f"Two-stage delay PPO expects a 3-D cmd_vel action, got {self.action_dim}"
            )
    ...
    def train(self, tensordict):
        ...
        # The environment continues advancing other vectorized instances after
        # one instance reaches its time limit. Treat both termination and
        # truncation as a boundary so GAE never bootstraps from that later state.
        dones = tensordict["next", "done"]
        time_scale = (
            tensordict["next", "stats", "transition_dt"]
